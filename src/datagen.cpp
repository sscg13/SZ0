...
int generate_mcts_selfplay_game(NNEvaluator &nn,
  ...
  while (true) {
    ...
    arena.clear();
    search_position(nn, arena, pos, game_hashes, 0, nodecount, 1, false);
    Node &root = arena.nodes[0];
    if (root.num_children == 0) {
      game_result = pos.stm ? 1 : -1;
      break;
    }
    // Calculate total visits across all children to form the probability
    // distribution
...
void datagen_worker(int thread_id, int node_limit, int game_count,
  ...
  std::string output_filename =
      base_filename + std::to_string(thread_id) + ".data";
  std::cout << "[Thread " << thread_id
            << "] Started playing. Writing to: " << output_filename << "\n";
  ...
  while (games_played < game_count) {
    positions_saved +=
        generate_mcts_selfplay_game(nn, output_filename, node_limit);
    games_played++;
    // Print an update every 10 games so you know it hasn't frozen
    if (games_played % 10 == 0) {
      std::cout << "[Thread " << thread_id << "] Completed " << games_played
                << " games and " << positions_saved << " positions\n";
    }
  }
}